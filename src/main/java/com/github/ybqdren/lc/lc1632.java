...
public class lc1632 {
    public int[][] matrixRankTransform(int[][] matrix) {
        ...
        for (int i =0 ; i<m ;i++){
            Map<Integer, List<int[]>> nums2indexList = new HashMap<Integer, List<int[]>>();
            for (int j =0 ; j<n ; j++){
                int num = matrix[i][j];
                nums2indexList.putIfAbsent(num , new ArrayList<int[]>());
                nums2indexList.get(num).add(new int[]{i,j});
            }

            for (List<int[]> indexList : nums2indexList.values()){
